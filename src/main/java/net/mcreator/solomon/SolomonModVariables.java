...
public class SolomonModVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "solomon_mapvars";
		public double direction = 0;
		public double shashengyingnum = 0;
		public boolean yaealive = false;
		public double shogunspdir = 0;
		public double wuxiangdire = 0;
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void readFromNBT(NBTTagCompound nbt) {
			direction = nbt.getDouble("direction");
			shashengyingnum = nbt.getDouble("shashengyingnum");
			yaealive = nbt.getBoolean("yaealive");
			shogunspdir = nbt.getDouble("shogunspdir");
			wuxiangdire = nbt.getDouble("wuxiangdire");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setDouble("direction", direction);
			nbt.setDouble("shashengyingnum", shashengyingnum);
			nbt.setBoolean("yaealive", yaealive);
			nbt.setDouble("shogunspdir", shogunspdir);
			nbt.setDouble("wuxiangdire", wuxiangdire);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				SolomonMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				SolomonMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "solomon_worldvars";
		public boolean asdsd = false;
		...
		@Override
		public void readFromNBT(NBTTagCompound nbt) {
			asdsd = nbt.getBoolean("asdsd");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setBoolean("asdsd", asdsd);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				SolomonMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				SolomonMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					SolomonMod.PACKET_HANDLER.sendToAll(message);
				else
					SolomonMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
